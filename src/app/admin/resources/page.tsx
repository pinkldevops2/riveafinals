...
export default function Resources() {
    ...
    const fetchData = async (pageNumber = 1, perPageSize = 10, sortCol = "_id", sortOrd = "asc", search = '') => {
        //setLoading(true);
        try {
            getResources(search, pageNumber, perPageSize, sortCol, sortOrd).then(result => {
                if (result.data) {
                    setData(result.data);
      ...
      const handleSort = (column: any, sortDirection: string) => {
        ...
        const columns = [
            ...
            { name: "Author",  selector: (row: any) => row.name, sortable: true, sortKey: 'name' },
            ...
            cell: (row: any) => (
                ...
                {row.image != undefined && row.image != '' ? (
                  <>
                  {<div className='mr-3 p-2'><img src={serverUrl+'/'+row.image} width={48}  alt="Image of the resource"/></div>}
                  </>
                ) : (
                  <>
                  <div className='mr-3 p-2'><img src={serverUrl+'/uploads/images/undraw_profile.svg'} width={48}  alt="Image of the resource"/></div>
                  </>
                )}
                </>
            ...
            cell: (row: any) => (
                <>
                <Link className="btn text-primary" href={"/admin/resources/"+row._id}>
                    <i className='fa fas fa-pen'></i>
                </Link>
                <ConfirmDeleteModal id={row._id} onDelete={handleDelete} msg="Are you sure you want to delete this resource?" />
                </>
            ),
            },
        ];
          ...
          <div id="content">
            ...
            <div className="container-fluid">
              ...
              <div className="d-sm-flex align-items-center justify-content-between mb-4">
                  <h1 className="h3 mb-0 text-gray-800">Resources</h1>
                  <div className="">
              ...
              <div className="row">
                  ...
                  <div className="card shadow mb-4">
                        <div className="card-header py-3 d-flex justify-content-between">
                            <h6 className="m-0 font-weight-bold text-primary">Resources</h6>
                            <Link className='btn btn-dark btn-sm' href={'/admin/resources/add'}><i className='fa fa-plus'></i> Add New Resource</Link>
                        </div>
