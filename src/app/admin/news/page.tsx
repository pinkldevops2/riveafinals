...
import { getNews, deleteNews } from '@/utils/news';
...
export default function News() {
    ...
    const fetchData = async (pageNumber = 1, perPageSize = 10, sortCol = "_id", sortOrd = "asc", search = '') => {
        //setLoading(true);
        try {
            getNews(search, pageNumber, perPageSize, sortCol, sortOrd).then(result => {
                if (result.data) {
                    setData(result.data);
        ...
        const columns = [
            ...
            cell: (row: any) => (
                ...
                {row.image != undefined && row.image != '' ? (
                  <>
                  {<div className='mr-3 p-2'><img className="" src={serverUrl+'/'+row.image} width={48} /></div>}
                  </>
                ) : (
                  <>
                  <div className='mr-3 p-2'><img src={serverUrl+'/uploads/images/undraw_profile.svg'} width={48} alt="Image of the news"/></div>
                  </>
                )}
                </>
            ...
            cell: (row: any) => (
                <>
                <Link className="btn text-primary" href={"/admin/news/"+row._id}>
                    <i className='fa fas fa-pen'></i>
                </Link>
                <ConfirmDeleteModal id={row._id} onDelete={handleDelete} msg="Are you sure you want to delete this news?" />
                </>
            ),
            },
        ];
              ...
              <div className="row">
                  ...
                  <div className="card shadow mb-4">
                        <div className="card-header py-3 d-flex justify-content-between">
                            <h6 className="m-0 font-weight-bold text-primary">News</h6>
                            <Link className='btn btn-dark btn-sm' href={'/admin/news/add'}><i className='fa fa-plus'></i> Add New News</Link>
                        </div>
